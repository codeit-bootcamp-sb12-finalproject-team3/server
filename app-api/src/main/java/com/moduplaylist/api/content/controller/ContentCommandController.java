...
public class ContentCommandController {
	...
	public ResponseEntity<ContentCreateResponse> create(
		...
		MultiValueMap<String, MultipartFile> imageFiles = new LinkedMultiValueMap<>();
		if (files != null) {
			imageFiles.addAll(files);
			imageFiles.remove("request");
		}
		if (imageFiles.values().stream().anyMatch(values -> values.size() != 1)) {
			throw new InvalidContentSearchException();
		}
		Map<String, MultipartFile> images = imageFiles.toSingleValueMap();
