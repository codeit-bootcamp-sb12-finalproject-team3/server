...
public class ContentPreferenceScoreRepository {

    private static final String TAG_SCORE_UPSERT = """
            INSERT INTO user_content_tag_preferences(id, user_id, tag_id, score, updated_at)
    ...
    private static final String GENRE_SCORE_UPSERT = """
            INSERT INTO user_content_genre_preferences(id, user_id, genre_id, score, updated_at)
            VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP(6))
            ON DUPLICATE KEY UPDATE
                score = score + ?,
                updated_at = CURRENT_TIMESTAMP(6)
